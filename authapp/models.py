...
class CustomUser(AbstractBaseUser, PermissionsMixin):
    ...
    )#поле avatar для загрузки фотографии пользователя.
    ...
    )#Поле email теперь обязательно для заполнения и уникально
    ...
    def users_avatars_path(instance, filename):
        ...
        # MEDIA_ROOT / user_<username> / avatars / <filename>
        num = int(time() * 1000)
        suff = Path(filename).suffix #filename — имя загруженного файла
        return "user_{0}/avatars/{1}".format(instance.username, f"pic_{num}{suff}")
